...
|------|---------|-------------|
| `eq` | `{ operator: 'eq', left: 'A', value: 1 }` | Equality with literal |
| `eq` (right) | `{ operator: 'eq', left: 'A', right: 'B' }` | Equality with another field |
| `ne` | `{ operator: 'ne', left: 'A', value: 1 }` | Inequality |
| `gt`, `lt`, `gte`, `lte` | `{ operator: 'gt', left: 'A', value: 5 }` | Comparison operators |
| `in` | `{ operator: 'in', left: 'A', values: [1,2,3] }` | Set membership |
...
{ operator: 'or', conditions: [
  { operator: 'ne', left: 'A', value: 'x' },   // NOT A
  { operator: 'eq', left: 'B', value: 'y' },    // B
]}
```
